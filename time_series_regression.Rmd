...
* In order to measure the quality of the methods: re-simulate the training data ($K = `r Ksim`$) with varying seeds and $\rho$, then measure how often Type 1 errors occur ($\alpha=0.10$).
...
allSimulations <- map_dfr(c(`1-Low`=0.1, `2-Moderate`=0.6, `3-High`=0.99),
...
tbl <- allSimulations %>% 
  group_by(rho, method) %>% 
  summarize(Prob_Type_1_Error = mean(Type1ErrorFlag), .groups = 'drop') %>% 
...
* Under high serial correlation, most of the previous methods struggle to return the correct p-values.
