...
doOneSimulation <- function(randomSeed=2013, alpha=0.10, rho=0.6) {
  ...
  test_mse_lm <- data.frame(method='lm', Test_MSE = mse(actual=test_plot$y,
                                                           predicted=test_plot$ols))
  test_mse_gls <- data.frame(method='gls', Test_MSE = mse(actual=test_plot$y,
...
* If $\widehat{\rho}$ is close to -1 or 1, then check for non-stationary errors (e.g., Phillips-Ouliaris Cointegration Test; Pesaran-Shin-Smith Cointegration Test).
...
* When $\rho=1$ or $\rho=-1$, the errors are non-stationary (i.e. "random walk").
...
doOneSimulationNS <- function(randomSeed=2013, alpha=0.10) {
  ...
  test_mse_lm <- data.frame(method='lm', Test_MSE = mse(actual=test_plot$z,
                                                           predicted=test_plot$ols))
  test_mse_gls <- data.frame(method='gls', Test_MSE = mse(actual=test_plot$z,
                                                             predicted=test_plot$gls))
  test_mse_rw <- data.frame(method='rw', Test_MSE = mse(actual=test_plot$z,
                                                             predicted=test_plot$rw))
